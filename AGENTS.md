...
실행 절차:
1. Codex가 작업을 2~5개 단위로 분해하고 우선순위 지정
2. 워커가 구현/수정/반복 작업 수행
3. Codex가 결과 통합 후 게이트 실행
4. 실패 시 최대 2회 자동수정, 이후 플랜 재작성
5. 최종 보고: 변경 파일 / 검증 결과 / 남은 리스크

품질 규칙:
- 워커 산출물은 Codex 검수 전 확정 금지
- 게이트 미통과 상태에서 완료 보고 금지
- 모호한 요청은 3~6줄 실행계획 먼저 제시
